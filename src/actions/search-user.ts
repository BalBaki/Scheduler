...
export const searchUser = async (formData: UserSearchForm): Promise<FormState<'search', UserSearchForm>> => {
    const headerList = headers();
    const validatedForm = userSearchSchema.safeParse(formData);

    if (!validatedForm.success) return { search: false, errors: validatedForm.error.flatten().fieldErrors };
